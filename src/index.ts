 ...
 *
 * Tools:
 * - dallas_recent:   recent rows from a common Dallas dataset by friendly name
 * - dallas_query:    raw SoQL query against any www.dallasopendata.com resource id
 * - dallas_datasets: search the Dallas open-data catalogue
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'DataDallas');
}

const BASE = 'https://www.dallasopendata.com';
const UA = 'pipeworx-mcp-data-dallas/1.0 (+https://pipeworx.io)';

const DATASETS: Record<string, { id: string; label: string; date: string }> = {
  '311': { id: 'd7e7-envw', label: "311 Service Requests (Oct 2020–present)", date: 'created_date' },
  'crime': { id: 'qv6i-rri7', label: "Dallas Police Incidents", date: 'date1' },
  'permits': { id: 'e7gq-4sah', label: "Building Permits", date: 'issued_date' },
};
...
const tools: McpToolExport['tools'] = [
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object' as const,
      properties: {
        resource_id: { type: 'string', description: 'Socrata resource id, e.g. "qv6i-rri7".' },
...
async function socrataGet(path: string, apiKey?: string): Promise<unknown> {
  ...
  if (res.status === 429) throw new Error('upstream_throttled: www.dallasopendata.com rate limit (HTTP 429). Pass _apiKey (Socrata app token) for higher limits.');
...
async function recent(dataset: string, where: string | undefined, limit: number | undefined, apiKey?: string) {
  ...
  return { dataset: key, label: ds.label, resource_id: ds.id, sorted_by: `${ds.date} DESC`, count: Array.isArray(rows) ? rows.length : 0, source: 'DataDallas (www.dallasopendata.com)', rows };
}

async function query(args: Record<string, unknown>, apiKey?: string) {
  const id = String(args.resource_id ?? '').trim();
  if (!id) throw new Error('Required argument "resource_id" is missing. Find one with dallas_datasets.');
  const p = new URLSearchParams();
  ...
  return { resource_id: id, count: Array.isArray(rows) ? rows.length : 0, source: 'DataDallas (www.dallasopendata.com)', rows };
}

async function datasets(q: string | undefined, limit: number | undefined, offset: number | undefined, apiKey?: string) {
  const p = new URLSearchParams({
    domains: 'www.dallasopendata.com',
    search_context: 'www.dallasopendata.com',
